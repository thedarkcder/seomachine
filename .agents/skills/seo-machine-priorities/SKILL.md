---
name: seo-machine-priorities
description: "Use for SEO Machine priorities work in Codex. Generate a comprehensive, prioritized content roadmap using multiple SEO research angles."
---

# Priorities Skill

This is a Codex-native SEO Machine skill. Use it directly with `$seo-machine-priorities` and pass the same argument you would give the workflow, such as a topic, URL, file path, keyword, or options.
...
## Codex Invocation
`$seo-machine-priorities` - Comprehensive research (all modules, ~10 minutes)
`$seo-machine-priorities quick` - Quick wins only (fast, ~2 minutes)
...
- **Ranking URL**: https://yoursite.com/path/to$seo-machine-article
...
**Next Steps:**
- Use `$seo-machine-analyze-existing [URL]` for detailed analysis of existing content
- Use `$seo-machine-write [keyword]` to create new content targeting identified keywords
- Use `$seo-machine-optimize [file]` before publishing any updates
```
...
The skill creates:
- `research$seo-machine-priorities-YYYY-MM-DD.md` (summarized top 10 list)

## Integration
After running `$seo-machine-priorities`, you can:
- Use `$seo-machine-analyze-existing [URL]` on any of the existing content items
- Use `$seo-machine-write [keyword]` to create new content for identified opportunities
- Use `$seo-machine-optimize [file]` to finalize any content before publishing

This skill gives you a clear, actionable roadmap of exactly what to work on next, prioritized by potential impact.
