---
name: seo-machine-content-calendar
description: "Use for SEO Machine content-calendar work in Codex. Generate a dated, month-long content calendar mapped to topic clusters, keywords, and publishing cadence using existing research data."
---

# Content Calendar Skill

This is a Codex-native SEO Machine skill. Use it directly with `$seo-machine-content-calendar` and pass the same argument you would give the workflow, such as a topic, URL, file path, keyword, or options.
...
## Codex Invocation
`$seo-machine-content-calendar` - Generate calendar for current month (2 posts/week)
`$seo-machine-content-calendar [posts-per-week]` - Custom cadence (e.g., `$seo-machine-content-calendar 3`)
`$seo-machine-content-calendar [posts-per-week] [month]` - Custom cadence and month (e.g., `$seo-machine-content-calendar 3 next-month`)
`$seo-machine-content-calendar [posts-per-week] [month] [focus-cluster]` - Filter to a specific cluster (e.g., `$seo-machine-content-calendar 2 april "content marketing"`)

## What This Skill Does

Bridges the gap between `$seo-machine-priorities` (what to work on) and execution (when to publish). Produces a dated publishing schedule with:
...
- Exact skills to run for each piece
...
Save the calendar to `research$seo-machine-content-calendar-YYYY-MM.md` where YYYY-MM is the target month.
...
- **Skill**: `$seo-machine-rewrite [topic]` then `$seo-machine-optimize [file]`
...
| Date | Topic | Type | Cluster | Skill |
|------|-------|------|---------|---------|
| Mon, [Month] [DD] | [topic] | Quick Win | [cluster] | `$seo-machine-rewrite [topic]` |
| Thu, [Month] [DD] | [topic] | Trending | [cluster] | `$seo-machine-article [topic]` |
| Mon, [Month] [DD] | [topic] | New Article | [cluster] | `$seo-machine-article [topic]` |
| ... | | | | |
...
2. Run `$seo-machine-research [topic]` before each new article for deeper keyword research
3. Use `$seo-machine-priorities` mid-month to reassess based on new data
4. Re-run `$seo-machine-content-calendar [posts-per-week] [next-month]` at month end to plan ahead
```
...
The skill creates:
- `research$seo-machine-content-calendar-YYYY-MM.md` (month-based naming since it covers a full month)

## Integration

After running `$seo-machine-content-calendar`:
- Use the exact skills listed for each calendar entry
- Run `$seo-machine-priorities` mid-month to validate direction
- Re-run `$seo-machine-content-calendar` at month end to plan the next month
...
- **After `$seo-machine-priorities`**: Turn prioritized list into a dated schedule
