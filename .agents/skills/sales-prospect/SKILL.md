---
name: sales-prospect
description: "Use this Codex sales skill for AI SDR work. You are the full prospect audit engine for `$sales-prospect <url>`. You launch 5 specialist passes, aggregate their results, and produce a unified PROSPECT-ANALYSIS.md report that is ready-to-use and deal-focused."
---

# Full Prospect Analysis Orchestrator

This is a Codex-native sales skill in the `sales-*` namespace. Use it directly with `$sales-prospect` or route to it from `$sales`.
...
You are the full prospect audit engine for `$sales-prospect <url>`. You launch 5 specialist passes, aggregate their results, and produce a unified PROSPECT-ANALYSIS.md report that is ready-to-use and deal-focused.
...
Before launching specialist review passes, perform these discovery steps sequentially. Every subsequent phase depends on this data.

### 1.1 Fetch the Target URL

Use `web fetch or browser/search tools` to retrieve the company homepage. Store the full content for specialist consumption.
...
Classify the prospect into one of these categories. This classification shapes every specialist's analysis focus and scoring calibration:
...
2. Perform manual extraction using web fetch or browser/search tools data
3. Note in the report that automated extraction was unavailable

### 1.6 Compile Discovery Briefing

Before launching specialist review passes, compile a discovery briefing object containing:
...
This briefing is passed to every specialist as context.

---

## Phase 2: Parallel Analysis (5 Specialist Review Passes Simultaneously)

Run all 5 specialist review passes from the same discovery briefing. If Codex subagent tools are available and the user explicitly asks for parallel agent work, run them in parallel; otherwise perform the specialist passes locally in sequence.

**Important:** Each specialist pass is independent. Parallel execution is useful when explicitly requested, but sequential local execution is acceptable.

### Specialist 1: sales-company-specialist (Company Research & Firmographics)

**Skill file:** `.agents/skills/sales-research/SKILL.md`
...
### Specialist 2: sales-contacts-specialist (Decision Maker Intelligence)

**Skill file:** `.agents/skills/sales-contacts-specialist/SKILL.md`
...
### Specialist 3: sales-opportunity-specialist (Opportunity & Budget Assessment)

**Skill file:** `.agents/skills/sales-qualify/SKILL.md`
**Weight:** 20% of Prospect Score
...
### Specialist 4: sales-competitive-specialist (Competitive Positioning)

**Skill file:** `.agents/skills/sales-competitors/SKILL.md` (if available) or default competitive analysis
...
**Research methodology for the competitive specialist:**
...
### Specialist 5: sales-strategy-specialist (Outreach Strategy & Messaging)

**Skill file:** `.agents/skills/sales-outreach/SKILL.md`
**Weight:** 20% of Prospect Score
...
After all 5 specialist review passes complete, aggregate their results into the final analysis.

### 3.1 Handle Specialist Failures

If any specialist fails or times out:
1. Note the failure in the report: "[Specialist name] analysis unavailable — [reason]"
...
Based on the Prospect Score and specialist findings, create a three-tier action plan:
...
Using the outreach strategy specialist's findings, craft the actual first outreach email. This must be:
...
|-----------------|---------|
| **High** | All 5 specialist review passes completed successfully. Rich public data available. Multiple data sources confirmed findings. |
| **Medium** | 4 of 5 specialist review passes completed. Moderate public data. Some findings based on inference. |
| **Low** | 3 or fewer specialist review passes completed. Limited public data. Significant reliance on inference. |
...
[Full company research findings from the sales-company-specialist.
...
### Specialist Failure
1. Log which specialist failed and why
...
5. Continue with remaining specialist data
...
2. Supplement with web search for external data (Crunchbase, LinkedIn, news)
...
- If `COMPANY-RESEARCH.md` exists in the current directory, incorporate its findings instead of running the sales-company-specialist from scratch
