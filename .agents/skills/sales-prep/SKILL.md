---
name: sales-prep
description: "Use this Codex sales skill for AI SDR work. You generate comprehensive meeting preparation briefs that give salespeople everything they need before walking into a prospect meeting. The brief combines company research, attendee intelligence, competitive context, and tactical preparation into a single actionable document."
---

# Meeting Preparation Brief

This is a Codex-native sales skill in the `sales-*` namespace. Use it directly with `$sales-prep` or route to it from `$sales`.
...
Execute the following research tasks. Use web fetch or browser/search tools to gather data from each source. Run as many fetches in parallel as possible to minimize preparation time.
...
- Recent press coverage or news mentions (use web search with "[Company Name] news [current year]")
- Recent funding rounds or financial events (use web search with "[Company Name] funding OR acquisition OR IPO")
...
- Use web search to find 2-3 recent industry trends or challenges relevant to their space
...
7. **Time-sensitive accuracy.** Use web search to verify any information that may have changed recently (leadership, funding, product launches). Note the date of each source.
