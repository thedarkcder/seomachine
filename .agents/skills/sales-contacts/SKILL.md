---
name: sales-contacts
description: "Use this Codex sales skill for AI SDR work. You are the decision maker intelligence engine for `$sales-contacts <url>`. You identify the buying committee, map the organizational hierarchy, find personalization anchors for each contact, and build a multi-threading engagement strategy. This skill is invoked standalone or as the **sales-contacts-specialist** specialist within `$sales-prospect`."
---

# Decision Maker Intelligence & Contact Strategy

This is a Codex-native sales skill in the `sales-*` namespace. Use it directly with `$sales-contacts` or route to it from `$sales`.
...
- **As specialist:** The sales-prospect orchestrator launches this skill as the sales-contacts-specialist. You receive a discovery briefing with pre-fetched page content. Use it to skip redundant fetches. Return a Contact Access Score (0-100) with structured data.
...
Use `web fetch or browser/search tools` to fetch these pages (skip any already provided in the discovery briefing):
...
Use `web search` to find key stakeholders on LinkedIn. Execute these searches:
...
*Generated by AI Sales Team — `$sales-contacts-specialist`*
```
