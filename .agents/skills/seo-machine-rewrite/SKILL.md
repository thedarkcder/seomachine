---
name: seo-machine-rewrite
description: "Use for SEO Machine rewrite work in Codex. Use this skill to update and improve existing blog posts based on analysis findings."
---

# Rewrite Skill

This is a Codex-native SEO Machine skill. Use it directly with `$seo-machine-rewrite` and pass the same argument you would give the workflow, such as a topic, URL, file path, keyword, or options.
...
Use this skill to update and improve existing blog posts based on analysis findings.
...
- **Analysis Report**: Review findings from `$seo-machine-analyze-existing` if available
...
### Content Structure
Follow same structure as `$seo-machine-write` skill:
...
### Scrubbing Process
1. **Invoke Scrubber**: Run `$seo-machine-scrub [file-path]` on the saved rewritten article file
...
### Example Workflow
```
1. Rewrite article → Save to rewrites$seo-machine-article-name-rewrite-2025-10-31.md
2. IMMEDIATELY run: $seo-machine-scrub rewrites$seo-machine-article-name-rewrite-2025-10-31.md
...
3. Run `$seo-machine-optimize` for final polish if needed
